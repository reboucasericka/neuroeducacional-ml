...
def dashboard():
    ...
    recent_instruments = (
        db.session.query(
            AssessmentInstrument.instrument_name,
            AssessmentInstrument.instrument_short_name,
            func.count(AssessmentInstrument.id).label("uses"),
        )
        .join(Assessment, Assessment.id == AssessmentInstrument.assessment_id)
        .filter(Assessment.professional_id == current_user.id)
        .group_by(
            AssessmentInstrument.instrument_name,
            AssessmentInstrument.instrument_short_name,
        )
        .order_by(func.count(AssessmentInstrument.id).desc())
        .limit(5)
        .all()
    )
    recent_profile_updates = (
        db.session.query(
            CognitiveIndicator,
            Patient,
            CognitiveDomain,
        )
        .join(Patient, Patient.id == CognitiveIndicator.patient_id)
        .join(CognitiveDomain, CognitiveDomain.id == CognitiveIndicator.domain_id)
        .filter(CognitiveIndicator.professional_id == current_user.id)
    ...
    stats = {
        ...
        "assessments_completed": Assessment.query.filter_by(
            professional_id=current_user.id, status="completed"
        ).count(),
        "anamneses_open": PatientAnamnesis.query.filter_by(
            professional_id=current_user.id, status="draft"
        ).count(),
        "anamneses_completed": PatientAnamnesis.query.filter_by(
            professional_id=current_user.id, status="completed"
        ).count(),
...
def patients_list():
    ...
    return render_template("panel/patients_list.html", patients=patients, q=q)
...
def patients_new():
    if request.method == "POST":
        try:
            ...
            return render_template(
                "panel/patient_form.html", sex_choices=SEX_CHOICES, form=request.form
            )
        ...
        if exists:
            flash(f"Já existe um {subject_label(current_user).lower()} com esse código interno.", "error")
            return render_template(
                "panel/patient_form.html", sex_choices=SEX_CHOICES, form=request.form
            )
    ...
    return render_template("panel/patient_form.html", sex_choices=SEX_CHOICES, form={})
...
def patient_detail(patient_id: int):
    ...
    if tab == "feedbacks" or tab == "devolutiva":
    ...
    for assessment in assessments:
        items = assessment.instruments.order_by(AssessmentInstrument.id.asc()).all()
        for item in items:
            key = item.display_name
            instruments_used[key] = item.display_short_name or ""
    ...
    history = (
        PatientHistoryEntry.query.filter_by(patient_id=patient.id)
        .order_by(PatientHistoryEntry.recorded_at.desc())
        .limit(20)
        .all()
    )
    anamneses = (
...
def settings_placeholder():
    return render_template(
        "panel/placeholder.html",
        title="Configurações",
        message="Placeholder para preferências e segurança futura.",
    )
